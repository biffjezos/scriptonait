...
// Vocab is small (259, byte-level) so this is cheap even though it's
// O(vocab * t_len) instead of O(t_len), and - the actual point - it
// needs no atomics, unlike a literal per-token scatter would (byte-level
// vocab guarantees token collisions within any T longer than 259).
struct Params {
    t_len: u32,
    hidden: u32,
    vocab: u32,
